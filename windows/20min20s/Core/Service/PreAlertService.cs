...
namespace ProjectEye.Core.Service
{
    ...
    public class PreAlertService : IService
    {
        ...
        public void InitPreAlert()
        {
            ...
            if (config.options.Style.IsPreAlert)
            {
                double remainingSeconds = main.GetRestCountdownMinutes() * 60;
                double dueAfterSeconds = remainingSeconds - config.options.Style.PreAlertTime;
                if (dueAfterSeconds <= 0)
                {
                    return;
                }
                ...
                preAlertTimer.Interval = TimeSpan.FromSeconds(dueAfterSeconds);
        ...
        private void preAlertTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (main.ShouldDeferBreakReminder())
            {
                return;
            }
            else
